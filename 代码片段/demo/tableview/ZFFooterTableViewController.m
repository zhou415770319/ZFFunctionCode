...
#import "ZFFooterTableViewController.h"
#import "ZFTableView.h"
#import "AutolayoutView.h"

@interface ZFFooterTableViewController ()
//显示数据
...
@implementation ZFFooterTableViewController
...
- (void)viewDidLoad {
    ...
    self.tableView.FooterView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"3"]];
    //间距为10 实际row 高度60
...
-(void)addFooter{
    UIView *headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.tableView.frame.size.width, self.tableView.heightForFooter)];
    headerView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"7"]];

    headerView.alpha =0.97;
    UILabel *headerLab = [AutolayoutView autolayoutLableWithTitle:@"wowowowo"];
    ...
    self.tableView.FooterView = headerView;
